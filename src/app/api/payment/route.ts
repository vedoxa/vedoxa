...
export async function POST(req) {
  try {
    // FIX: Connection ab function ke ANDAR hai, isliye Vercel error nahi dega
    ...
    if (body.action === 'create_order') {
      ...
      // Razorpay se order ID generate karo
      const order = await rzp.orders.create({ amount: finalPrice * 100, currency: "INR" });
      return NextResponse.json({ rzpOrderId: order.id, amount: finalPrice, realBookPrice: book.final_price });
    }

    // 2. VERIFY SIGNATURE & SAVE TO DATABASE
    if (body.action === 'verify') {
      ...
      if (expectedSign === rzpSignature) {
        // Payment Asli Hai -> Ab database me safe entry karo
        await supabaseAdmin.from('orders').insert([{ ...orderData, payment_status: 'completed' }]);
